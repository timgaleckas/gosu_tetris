class Sprites
  ...
  SQUARES = 0.upto(SQUARES_SHEET.height / SQUARE_HEIGHT - 1).map do |row|
    0.upto(SQUARES_SHEET.width / SQUARE_WIDTH - 1).map do |column|
      SQUARES_SHEET.subimage(SQUARE_WIDTH*column,SQUARE_HEIGHT*row,SQUARE_WIDTH,SQUARE_HEIGHT)
    end
  end

  HORIZONTAL_STITCHES = 0.upto(HORIZONTAL_STITCHES_SHEET.height / STITCH_WIDTH - 1).map do |row|
    0.upto(HORIZONTAL_STITCHES_SHEET.width / (SQUARE_HEIGHT - 2) - 1).map do |column|
      HORIZONTAL_STITCHES_SHEET.subimage((SQUARE_WIDTH-2)*column,STITCH_WIDTH*row,SQUARE_WIDTH-2,STITCH_WIDTH)
    end
  end

  VERTICAL_STITCHES = 0.upto(VERTICAL_STITCHES_SHEET.height / (SQUARE_HEIGHT - 2) - 1).map do |row|
    0.upto(VERTICAL_STITCHES_SHEET.width / STITCH_WIDTH - 1).map do |column|
